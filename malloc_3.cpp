...
void* smalloc(size_t size){
	...
	if(size >= 128*1024){
		MallocMetadata* result = allocate_space(size);
    ...
    if (result) {
        ...
        return challenge1(result,size);
    }
  //check for the wilderness block
	result = challenge3(size);
...
void* srealloc(void* oldp, size_t size){
	...
	if(size >= 128*1024 && ((MallocMetadata*) oldp - 1)->req_size >= 128*1024 ){
		MallocMetadata* freedblock = ((MallocMetadata*) oldp - 1);
		MallocMetadata* new_block = (MallocMetadata*) mmap(NULL, size+ sizeof(class MallocMetadata),
					PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0);
	    if((void*) new_block == (void *) -1) return (NULL);
		//std::memcpy(new_block, oldp, size);
		new_block->req_size=size;
		 size_t oldsize=freedblock->req_size+META_SIZE;
		 freedblock->req_size=size;
		new_block->next=NULL;
		new_block->prev=NULL;
	...
	if((block_ptr->req_size) >= size) return oldp;

	if((size > (block_ptr->req_size))) return reallocWilderness(oldp,block_ptr,size);// && (!(block_ptr->next) || (block_ptr->next && block_ptr->next->is_free ))
    void *result = smalloc(size);
    if (result == (void *) NULL) {
        return result;
    }
